...
//! (see `gui-aax`'s `page_table` module for why this resource is generated
...
//! only touches `gui_aax::page_table`, which has no SDK dependency), so it
//! can't simply reuse the `extern "C"` getters `aax_entry!` generates.
use gui_host::Processor as _;

fn main() {
    let identity = gui_aax::page_table::AaxPluginIdentity {
    ...
    let xml = gui_aax::page_table::generate_page_table_xml(&identity, processor.parameters());
    print!("{xml}");
}
