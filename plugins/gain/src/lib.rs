...
//! `mkapk-vst3`, not authored here), which is why this crate does not set
//! `#![deny(unsafe_code)]` at the root the way `mkapk-standalone` does.
...
// different plugin). `mkapk_vst3::vst3_entry!` only exists (and only expands
// to anything, including the `unsafe extern "system"` VST3 module-load
// exports it generates) when mkapk-vst3 is built with `VST3_SDK_PATH` set.
...
// `mkapk-vst3`'s `IAudioProcessor::process`, which isn't wired to a shared
// `PeakMeter` the way `mkapk-standalone`'s audio callback is), so the meter
...
// macOS), so `mkapk_au::au_entry!` only needs `target_os = "macos"` to have
// gated the real entry point in, not an `_SDK_PATH` env var; see
// `mkapk-au`'s `component` module for the real audio-thread wiring this
...
// AAX's own "Describe"/parameter-registration code (`mkapk-aax/cpp/`) is a
...
// in C++ at all. See `mkapk-aax::component`'s doc comment for the full
// picture.
